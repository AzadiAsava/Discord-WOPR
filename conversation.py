...
from chatgpt import find_similar_conversations, merge_conversations, send_to_ChatGPT
...
class Mode:
    def __init__(self, name : str, user : int = -1):
        ...
        self.user = user
    @abstractmethod
    async def __call__(self, user : int, conversation : "Conversation", db) -> str:
...
class DefaultMode(Mode):
    ...
    async def __call__(self, message : str, conversation : "Conversation", db) -> str:
        return message
...
class DateTimeAwareMode(Mode):
    ...
    async def __call__(self, message : str, conversation : "Conversation", db) -> str:
        timezone = db.get_preference(self.user, "timezone")
...
class KnowledgeAwareMode(Mode):
    def __init__(self, user : int, update_interval : int = 4):
    ...
    async def __call__(self, message : str, conversation : "Conversation", db) -> str:
        if self.update_counter >= self.update_interval:
            ...
            for conversation in conversations.values():
                ...
                db.set_conversation(self.user, conversation)
            summary = ""
            for i, conversation in enumerate(conversations.values()):
                summary += "Conversation " + str(i) + ": " + conversation.summary + "\n\n"
            ...
            while similar is not None:
                ...
                for i, conversation in enumerate(conversations.values()):
                    summary += "Conversation " + str(i) + ": " + conversation.summary + "\n\n"
                similar = await find_similar_conversations(summary)
            db.set_knowledge(self.user, summarize_knowledge(summary))
...
class Conversation:
    ...
    def get_conversation(self) -> List[dict[str, str]]:
        ...
        return messages
    async def add_user(self, user : str, db) -> None:
...
async def compress_conversation(conversation : Conversation, keep_last:int=6) -> Conversation:
    ...
    revised_convo = await summarize(convo)
